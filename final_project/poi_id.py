...
with open("../README.md", "a") as readme:
    readme.write("### Validation and Evaluation\n\n"
                 ...
                 "get a trustworthy accuracy number. A classic mistake in machine learning without the "
                 "cross-validation is that both the train and test are performed based on the same dataset, "
                 "the model that trained would overfit, which means the model has just seen would have a perfect "
                 "score but would fail to predict anything useful on yet-unseen data. So that the dataset should be "
                 "split into a known dataset(train) and an unknown dataset(test) to test the real performances of "
                 "the model.\n"

                 "Furthermore, the POI ratio of the dataset is as low as 12.3%, "
                 ...
                 "In this project's context, a true positive is that one identified as POI who is truly a POI, "
                 "while a false positive is that one identified as non-POI who is truly a POI, and a false negative "
                 "is that one identified as non-POI who is truly a non-POI. "
                 "The definition equations are as below.\n"
                 "- Precision = those identified as POI and truly are / (those identified as POI and truly are + "
                 "those identified as non-POI but they are POI)\n"
                 "- Recall = those identified as POI and truly are / (those identified as POI and truly are + "
                 "those identified as non-POI who is truly a non-POI)\n\n"
