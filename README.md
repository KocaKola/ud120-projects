...
**Why validation is important?** In the section above, several algorithms are tuned for better performance. The goal of cross validation is to define a dataset to test the model in the training phase. Without it, the overfit problem could not be aware. So, all the features and labels are split into the training set and test set for validation. The cross validation could help to reduce the impact of the overfit problem, thus, we could get a trustworthy accuracy number. A classic mistake in machine learning without the cross-validation is that both the train and test are performed based on the same dataset, the model that trained would overfit, which means the model has just seen would have a perfect score but would fail to predict anything useful on yet-unseen data. So that the dataset should be split into a known dataset(train) and an unknown dataset(test) to test the real performances of the model.
Furthermore, the POI ratio of the dataset is as low as 12.3%, in such cases using stratified sampling as implemented in StratifiedShuffleSplit to ensure that relative class frequencies is approximately preserved in each train and validation fold.

**Why evaluation metrics?** Also, during the tuning process, accuracy, precision and recall are employed as the benchmarks. With these benchmarks, the performance of the algorithms could be precisely qualified. Precision is the ratio of true positives and the sum of true positives and false positives, while recall is the ratio of true positives and the sum of true positives and false negatives. In this project's context, a true positive is that one identified as POI who is truly a POI, while a false positive is that one identified as non-POI who is truly a POI, and a false negative is that one identified as non-POI who is truly a non-POI. The definition equations are as below.
- Precision = those identified as POI and truly are / (those identified as POI and truly are + those identified as non-POI but they are POI)
- Recall = those identified as POI and truly are / (those identified as POI and truly are + those identified as non-POI who is truly a non-POI)
